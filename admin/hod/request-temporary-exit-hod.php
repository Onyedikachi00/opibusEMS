    ...
    $hodid = $_SESSION['aid'];
        ...
        if (isset($_POST['submit'])) {
            ...
            foreach ($_FILES['files']['name'] as $key => $name) {
                ...
                if (move_uploaded_file($tmp_name, "../request/hodTemporaryExit/$file_name")) {
                    $fileNames[] = $file_name; 
                }
            }
            ...
            $query = mysqli_query($con, "INSERT INTO tbltemporaryexit (Name, Email, PhoneNumber, StartTime, EndTime, Reason, UserType, Department, Substitute, filename) VALUES ('$hodName', '$hodEmail', '$hodMobile', '$sTime', '$eTime', '$lReason', '$utype', '$dept', '$subs', '$fileNamesString')");
            $last_id = $con->insert_id; 
            
            if($query){
              // echo "<script>alert('Request Sent successfully.');</script>";
              echo "<script type='text/javascript'> document.location = 'hod-temporary-exit-requests.php'; </script>";
            } else {
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Request Temporary Exit</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
