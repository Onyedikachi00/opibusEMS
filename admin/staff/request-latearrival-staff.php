...
if(strlen($_SESSION['aid'])==0)
  ...
  $getUserQuery = mysqli_query($con, "SELECT * FROM stafftable WHERE ID = '$staffid'");
  $result = mysqli_fetch_assoc($getUserQuery);
  $staffName = $result['Name'];
  $staffUname = $result['UserName'];
  ...
  if (isset($_POST['submit'])) {
      ...
      $staffUname = mysqli_real_escape_string($con, $_POST['uname']);
      ...
      $query = mysqli_query($con, "INSERT INTO tbllaterequest (Name, UserName, Email, PhoneNumber, Date, Time, Reason, UserType, Department) VALUES ('$staffName', '$staffUname', '$staffEmail', '$staffMobile', '$lDate', '$aTime', '$lReason', '$utype', '$dept')");

      if($query){
        // echo "<script>alert('Request Sent successfully.');</script>";
        echo "<script type='text/javascript'> document.location = 'staff-latearrival-requests.php'; </script>";
      } else {
              ...
              <form name="subadmin" method="post">
                <div class="card-body">
                    ...
                    <div class="form-group" style="display:none;">
                        <label for="exampleInputName">Name</label>
                        <input type="text" class="form-control mb-4" name="uname" style="font-weight:bolder; font-size:18px; color: black;" readonly value="<?php echo $staffUname?>">
                    </div>
                    <div class="form-group">
