    ...
    $staffid = $_SESSION['aid'];
        ...
        if (isset($_POST['submit'])) {
            ...
            foreach ($_FILES['files']['name'] as $key => $name) {
                ...
                if (move_uploaded_file($tmp_name, "../request/StaffLeave/$file_name")) {
                    $fileNames[] = $file_name; 
                }
                else{
                  echo "None";
            ...
            $query = mysqli_query($con, "INSERT INTO tblleaverequest (Name, Email, PhoneNumber, StartTime, EndTime, Reason, UserType, Department, Substitute, filename) VALUES ('$staffName', '$staffEmail', '$staffMobile', '$sTime', '$eTime', '$lReason', '$utype', '$dept', '$subs', '$fileNamesString')");
            $last_id = $con->insert_id;

            // $sql = "SELECT * FROM tblleaverequest";
            // $result = mysqli_query($con, $sql);

            // while ($row = mysqli_fetch_assoc($result)) {
            //     $subs = explode(',', $row['Substitute']); 

            //     $sentBy = $staffName;
            //     $content = "Hi Buddy, it's time"; // Pre-written text

            //     foreach ($subs as $sentTo) {
            //         $sqlUpdate = "INSERT INTO tblmessages (Name, SentBy, MailContent) VALUES ('$sentTo', '$sentBy', '$content')";
            ...
            if($query){
              ...
              echo "<script type='text/javascript'> document.location = 'staff-leave-requests.php'; </script>";
            } else {
